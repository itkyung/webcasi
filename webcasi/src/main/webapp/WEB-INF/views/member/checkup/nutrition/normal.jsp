...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script>
		...
		var _nextUrl = "${navigationInfo.nextUrl}";
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/script/member/nutrition/normal.js"></script>
</head>
<body>
		...
		<c:forEach var="questionRow" items="${questions}" varStatus="status">
			...
			<c:choose>
				<c:when test="${(questionRow.type eq 'CHECK_SUBJ') || (questionRow.type eq 'CHECK_SUBJ_SUBJ') ||
					...
					<c:set var="vertical" value="true" scope="request"/>
					<c:set var="noChild" value="true" scope="request"/>
					<jsp:include page="/WEB-INF/views/member/checkup/template/checkboxSubj.jsp" />
					<c:remove var="question" scope="request"/>
					<c:remove var="vertical" scope="request"/>
					<c:remove var="noChild" scope="request"/>
				</c:when>	
			</c:choose>
		</c:forEach>
		<div class="nutrition-button">
			<a href="javascript:showDepth();"><img src="${pageContext.request.contextPath}/resources/images/bt_ok.png"/></a>
