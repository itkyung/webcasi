...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/script/member/nutrition/habit.js"></script>
</head>
<body>
		...
		<c:forEach var="questionRow" items="${questions}" varStatus="status">
			<c:choose>
				...
				<c:when test="${questionRow.type eq 'TAB'}">
					<c:set var="question" value="${questionRow}" scope="request"/>
					<jsp:include page="/WEB-INF/views/member/checkup/template/tab.jsp" />
					<c:remove var="question" scope="request"/>					
				</c:when>										
			</c:choose>
		</c:forEach>
